...
class FirstModel:
    ...
    def __init__(self, inp_w, inp_h, inp_d, keep_prob_fc = 0.5, keep_prob_conv = 0.7):
        ...
        # self._inp_dropout = tf.nn.dropout(self._inp_norm, keep_prob_conv)
        self._Wconv1 = tf.get_variable("Wconv1", shape = [3, 3, inp_d, 32], initializer = tf.contrib.layers.xavier_initializer())
        ...
        # self._h1_drop_out = tf.nn.dropout(self._h1, keep_prob_conv)

        # Second convolutional layer:
        self._Wconv2 = tf.get_variable("Wconv2", shape = [3, 3, 32, 64], initializer = tf.contrib.layers.xavier_initializer())
        ...
        self._h2_dropout = tf.nn.dropout(self._h2_max_pool, keep_prob_conv)

        # Third convolutional layer:
        self._Wconv3 = tf.get_variable("Wconv3", shape = [3, 3, 64, 128], initializer = tf.contrib.layers.xavier_initializer())
        self._bconv3 = tf.get_variable("bconv3", shape = [128], initializer = tf.contrib.layers.xavier_initializer())
        self._z3 = tf.nn.conv2d(self._h2_dropout, self._Wconv3, strides = [1, 1, 1, 1], padding = 'VALID') + self._bconv3
        self._h3 = tf.layers.batch_normalization(tf.nn.relu(self._z3), axis = 1, training = self._is_training)
        # self._h3_dropout = tf.nn.dropout(self._h3, keep_prob_conv)

        # Fourth convolutional layer:
        self._Wconv4 = tf.get_variable("Wconv4", shape = [3, 3, 128, 256], initializer = tf.contrib.layers.xavier_initializer())
        self._bconv4 = tf.get_variable("bconv4", shape = [256], initializer = tf.contrib.layers.xavier_initializer())
        ...
        self._h4_dropout = tf.nn.dropout(self._h4_max_pool, keep_prob_conv)
        self._h4_flat = tf.reshape(self._h4_dropout, shape = [-1, 6400])

        # First fully-connected layer:
        self._W1 = tf.get_variable("W1", shape = [6400, 1024], initializer = tf.contrib.layers.xavier_initializer())
        self._b1= tf.get_variable("b1", shape = [1024])
        self._fc1 = tf.matmul(self._h4_flat, self._W1) + self._b1
        self._a2 = tf.nn.relu(self._fc1)
        self._a2_normalized = tf.layers.batch_normalization(self._a2, axis = 1, training = self._is_training)

        # Dropout:
        self._a2_dropout = tf.nn.dropout(self._a2_normalized, keep_prob_fc)
        ...
        self._a3 = tf.matmul(self._a2_dropout, self._W2) + self._b2
    ...
    def train(self, X, y):
        ...
        print('Training')
        self.run_model(self._sess, self._op, self._mean_loss, X, y, 7, 64, 100, self._train_step)
